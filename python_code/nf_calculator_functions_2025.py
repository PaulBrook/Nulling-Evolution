...
def waterfall_and_average(data,mean,on_windows,excluded_bins):
    ...
    ax2.grid()
    plt.vlines(on_windows[0],-0.1,1.1,linestyles='dashed',lw=4,color='green')
    plt.vlines(on_windows[-1],-0.1,1.1,linestyles='dashed',lw=4,color='green')
    ax2.add_patch(Rectangle((on_windows[0],-0.1),(on_windows[-1]-on_windows[0]),1.2,facecolor='green',alpha = 0.1, zorder = 39))
    #plt.vlines(other_emission_window[0],-0.1,1.1,linestyles='dashed',lw=4,color='black')
    #plt.vlines(other_emission_window[-1],-0.1,1.1,linestyles='dashed',lw=4,color='black')
    for excluded in excluded_bins:
        ax2.add_patch(Rectangle((excluded,-0.1), 1, 1.2,facecolor='black',alpha = 0.1, zorder = 39))

    

    # If you want to highlight each bin as a shaded region (optional)
    #for off_bin in off_windows:
    #    ax2.add_patch(Rectangle((off_bin, -0.1), 1, 1.2, facecolor='darkorange', alpha=0.1, zorder=39))
