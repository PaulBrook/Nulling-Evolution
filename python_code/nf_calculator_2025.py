...
import nf_calculator_functions_2025 as nff
...
# Define the start and end of the on_window
on_window_begin = args.fb
on_window_end = args.lb

no_bins = args.bins

# Create a list or array of all bins (assuming bins are sequential integers)
all_bins = list(range(no_bins))

# Create the on_window as a range of bins between on_window_begin and on_window_end
on_windows = list(range(on_window_begin, on_window_end + 1))
num_on_windows = len(on_windows)

on_window_begin = on_windows[0]
on_window_end = on_windows[-1]

other_emission_bins = []

if p_name == 'J1413-6307':
    other_emission_bins = list(range(350, 390+1))
if p_name == 'J1745-3040':
    other_emission_bins = list(range(101, 139+1)) # covers both major 114 to 139 and minor 101 to 113 on window
if p_name == 'J1825-0935':
    other_emission_bins = list(range(116, 138+1)) + list(range(380, 403+1))

# Create the off_window by excluding the on_window bins
#off_windows = [bin for bin in all_bins if bin not in on_windows]
off_windows = [
    bin for bin in all_bins
    if bin not in on_windows and bin not in other_emission_bins
]
#off_windows = list(range(140,191+1))

num_off_windows = len(off_windows)

print(f'NUM ON WINDOWS: {num_on_windows}')
print(f'NUM OFF WINDOWS: {num_off_windows}')

# Now off_windows contains all bins not included in the on_window
...
for file in os.listdir(datadir):
        ...
        for i in range(int(data.shape[0])):
            all_off_bins.append(data[i, off_windows])
            #all_off_bins.append(data[i,off_window_begin:off_window_end])

        all_off_bins = [item for sublist in all_off_bins for item in sublist]
        ...
        nff.waterfall_and_average(data,mean,on_windows,other_emission_bins);
        ...
        for i in range(data.shape[0]):
            ...
            # Fiand the max of data in the on_window bins
            max_data = np.max([data[i, bin] for bin in on_windows])            
            # Find the standard deviation of data in the off_window bins
            std_off_data = np.std([data[i, bin] for bin in off_windows])

            find_sn_threshold.append(max_data / std_off_data)
            #find_sn_threshold.append((np.max(data[i,on_window_begin:on_window_end])/np.std(data[i,off_window_begin:off_window_end])))
        ...
        for i in range(data.shape[0]):
            if find_sn_threshold[i] > sn_threshold:
            #if np.max(data[i,on_window_begin:on_window_end])/np.std(data[i,off_window_begin:off_window_end]) > sn_threshold:
                temp_sn_list.append(find_sn_threshold[i])
        ...
        for i in range(int(data.shape[0])):
            # Extract the data from the off_windows bins for each row in data
            off_window_data = [data[i, bin] for bin in off_windows]
            
            # Calculate the sum of the data in the off_windows bins
            #flux_density_off.append(np.sum(off_window_data))
            #flux_density_off.append((np.sqrt(num_on_windows)/np.sqrt(num_off_windows))*np.sum(off_window_data))
            flux_density_off.append((num_on_windows/num_off_windows)*np.sum(off_window_data))
            
            # Calculate the standard deviation of the data in the off_windows bins
            std_flux_density_off.append(np.std(off_window_data))
        ...
        print(f'list of all flux dens off: {flux_density_off}')
